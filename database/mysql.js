...
module.exports = function(){
  return{
    ...
    //회원 가입 컨트롤러
    join: function(conn, userid, userpw, username, callback)
    {
    ...
    join1: function(conn,userid, userpw, username, callback){
      ...
      if(username != ''&&userid!=''&&userpw!=''){
        if(no.test(username)||username.length>10){
        ...
        if(error){
          ...
          conn.query('update account set balance =? where accN=?',[result[0].balance, result[0].accN], function(error, data, field){
            if(error){
              ...
              conn.query('insert into F_trans(userid, useaccN,money, trans, inaccN, outaccN)values(?,? ,?,?,?)',[result[0].id, result[0].accN,money, "입금", result[0].accN], function(error, tr, field){
                if(error){
                  console.log(error);
                }
                console.log(result)
          ...
          if(result[0].balance < money){
            ...
            conn.query('update account set balance =? where accN=?',[result[0].balance, result[0].accN], function(error, data, field){
              if(error){
                ...
                conn.query('insert into F_trans(userid, useaccN,outaccN, money, trans, inaccN)values(?,?,?,?,?)',[result[0].id,result[0].accN, result[0].accN, -1*money, "출금"], function(error, tr, field){
                  if(error){
                    console.log(error);
                  }
                  console.log(result)
    ...
    get_t2: function(conn, myaccN,money,youraccN, callback){
    ...
    itch: function(conn, displayName, callback){
      var array;
      conn.query('select * from account where id like("'+displayName+'")', function(err, result, fields){
        array = new Array();
        for(var i =0; i<result.length; i++){
        ...
        i = 0;
        var list = '<h1>계좌 번호 조회></h1>';
        while(i < array.length){
          list = list + `<p><a herf="/main/menu/balance/?id=${array[i]}">${array[i]}</a></p>\n`;
          i = i+1;
        }
